...
| **Server-driven move ingress** (`Manager.DispatchServer` + enforced `Move.ServerOnly`) | Turn-timer force-end, disconnect-driven concede, scheduled admin actions, cron-job cleanup — server-originated moves without fake credentialed clients | None — users hand-roll their own paths |
